...
This is the code I want to refactor. It is intentionally messy and inconsistent to give us more to work with. The goal is to improve readability, maintainability, and robustness while preserving the original behavior (including quirks).
...
I want to refactor the agents to work together to clean this up. The conductor can coordinate the overall process, the critic can review the code and suggest improvements, the planner can create a step-by-step plan for refactoring, the refactorer can execute the plan, and the tester can ensure we have good test coverage to maintain behavior.

Try to keep the updates to the agents to be agnotic to the specific code, so we can reuse them for future refactoring tasks. For example, the planner can create a general plan template that can be applied to any codebase, and the critic can have a checklist of common code smells and issues to look for.